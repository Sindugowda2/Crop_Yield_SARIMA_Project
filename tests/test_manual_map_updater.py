import csv
from src.manual_map_updater import (
    suggest_and_apply,
...
def test_suggest_and_apply_dry_run(tmp_path, monkeypatch):
    ...
    with (proc / "missing_year_state_counts.csv").open(
    ...
    raw = tmp_path / "raw"
    raw.mkdir()
    with (raw / "rainfall_validation.csv").open(
        "w", newline="", encoding="utf-8"
    ) as fh:
    ...
    suggestions = suggest_and_apply(
        apply=False, raw_dir=str(raw), processed_dir=str(proc)
    )
