...
def main():
    ...
    try:
        ...
        crop_col = [c for c in df.columns if 'crop' == c.lower() or c.lower() == 'crop'][0]
        ...
        if ts.empty:
            ...
            print("PASS: time-series builder produced a series sample (length: {} )".format(len(ts)))
        ...
        try:
            ...
            if 'andaman and nicobar islands' in states and 'arecanut' in crops:
                ...
                if ts2.empty:
                    print("FAIL: Known sample (Andaman/Arecanut) produced empty series.")
                    sys.exit(6)
                else:
                    print("PASS: Known sample (Andaman/Arecanut) produced a non-empty series (length: {})".format(len(ts2)))
        except Exception as e:
