...
def test_build_action_plan(tmp_path):
    ...
    suit = pd.DataFrame([
        {"state_name":"odisha","crop":"rice","year_count":2,"usable_for_sarima":False},
        {"state_name":"andhra pradesh","crop":"rice","year_count":6,"usable_for_sarima":True},
    ])
    suit.to_csv(proc / 'dataset_suitability.csv', index=False)

    # fake missing counts
    miss = pd.DataFrame([
        {"state_name":"odisha","count":6244}
    ])
