...
def load_manual_map(path):
    ...
    try:
        ...
        if 'state' not in df.columns or 'subdivision' not in df.columns:
            print(f"WARN: Manual map file {path} missing required columns 'state' and 'subdivision'")
            return None
...
def main():
    ...
    rain_small = rain[['subdivision','year','kharif_rain','rabi_rain','whole_year_rain']].drop_duplicates()

    # make a simple mapping try exact merge first
    print("Merging datasets (state -> subdivision) ...")
    merged = crop.merge(rain_small, left_on='state_name', right_on='subdivision', how='left')
    ...
    if missing_years > 0:
        ...
        if csv_map:
            manual_map = csv_map
            print(f"Loaded manual map from {MANUAL_MAP_FILE} ({len(manual_map)} entries)")
        else:
            manual_map = {
                'odisha': 'orissa',
        ...
        merged.loc[need_map_idx, 'subdivision_manual'] = merged.loc[need_map_idx, 'state_name'].map(manual_map)
        # merge manual mapped subdivisions
        merged = merged.merge(rain_small, left_on='subdivision_manual', right_on='subdivision', how='left', suffixes=('','_manual'))
        # prefer existing year, then manual
        if 'year_manual' in merged.columns:
            merged['year'] = merged['year'].fillna(merged['year_manual'])
        merged['kharif_rain'] = merged['kharif_rain'].fillna(merged.get('kharif_rain_manual'))
        merged['rabi_rain'] = merged['rabi_rain'].fillna(merged.get('rabi_rain_manual'))
        merged['whole_year_rain'] = merged['whole_year_rain'].fillna(merged.get('whole_year_rain_manual'))
        # drop helper manual cols
        merged = merged.drop(columns=[c for c in merged.columns if c.endswith('_manual')])
        ...
        if len(still_missing_states) > 0:
            ...
            for s in still_missing_states:
                ...
                candidates = rain_small[rain_small['subdivision'].str.contains(s, na=False)]['subdivision'].unique()
                if len(candidates) == 1:
            ...
            if corrections:
                print(f"Found fuzzy matches for {len(corrections)} states. Applying corrections...")
                ...
                merged = merged.merge(rain_small, left_on=['subdivision_fuzzy'], right_on=['subdivision'], how='left', suffixes=('','_fuzzy'))
                # prefer exact year, then fuzzy
                if 'year_fuzzy' in merged.columns:
                    merged['year'] = merged['year'].fillna(merged['year_fuzzy'])
                merged['kharif_rain'] = merged['kharif_rain'].fillna(merged.get('kharif_rain_fuzzy'))
                merged['rabi_rain'] = merged['rabi_rain'].fillna(merged.get('rabi_rain_fuzzy'))
                merged['whole_year_rain'] = merged['whole_year_rain'].fillna(merged.get('whole_year_rain_fuzzy'))
                merged = merged.drop(columns=[c for c in merged.columns if c.endswith('_fuzzy')])
    ...
    print("Done. Review the CSV and run tests; if many rows are missing YEAR, we should add a manual mapping table.")
